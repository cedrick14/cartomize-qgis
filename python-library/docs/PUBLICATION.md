...
Les fichiers attendus sont `cartomize-0.8.0a1-py3-none-any.whl` et
`cartomize-0.8.0a1.tar.gz`. Tester l'installation du wheel dans un environnement
...
python -m pip install --index-url https://test.pypi.org/simple/ --no-deps cartomize==0.8.0a1
```
...
explicite `python -m pip install cartomize==0.8.0a1`. Les versions alpha ne sont
