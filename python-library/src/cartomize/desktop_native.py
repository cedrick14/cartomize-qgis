...
class NativePage(Page):
    ...
    def __init__(self,directory):
        ...
        for label,value in [('Inventaire du projet','inspect'),('Exporter une mise en page','export'),('Enregistrer une copie du projet','copy')]:self.action.addItem(label,value)
    ...
    def import_layers(self):
        ...
        for item in (self.inventory or {}).get('layers',[]):
            ...
            if source and not provider_options and Path(source).is_file() and item.get('visible',True) and Path(source).suffix.lower() in {'.tif','.tiff','.jp2','.vrt','.img','.gpkg','.shp','.geojson'}:layers.append(dict(data=source,name=item['name']))
