...
from ..core.community_catalog import CommunityCatalogClient, CommunityCatalogSnapshot
...
from .guided_tour import GuidedTourController, GuidedTourStep
...
class CartomizeDock(QDockWidget):
    def __init__(self, iface, plugin_root: Path, parent=None):
        ...
        self.community_catalog = CommunityCatalogClient(
            self.settings.community_url,
            self.community_cache_path,
        )
        self.community_catalog_task = None
        self.current_report = None
        self.current_vector_profile = None
        self.current_style_recommendation = None
        ...
        self.brand_header = QFrame()
        self.brand_header.setObjectName("CartomizeBrandHeader")
        self.brand_header.setProperty("role", "brandHeader")
        header_layout = QHBoxLayout(self.brand_header)
        header_layout.setContentsMargins(12, 10, 12, 10)
        header_layout.setSpacing(10)

        icon_label = QLabel()
        icon_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # Une icône de 56 px ne justifie pas le décodage du grand bitmap HD.
        # Le petit asset réduit le pic mémoire au moment précis où le dock
        # avancé construit déjà ses nombreux contrôles sous Qt 5.
        ...
        self.footer_status = QLabel("Chargement progressif de l’interface…")
        ...
        self._tab_build_index = 0
        # Chaque page est créée dans un tour distinct de la boucle Qt. Cette
        # construction coopérative évite un long blocage de l'interface et le
        # watchdog graphique de Windows avec QGIS 3.40/Qt 5.
        QTimer.singleShot(0, self._build_next_tab)

    def _build_next_tab(self):
        if self._disposed or self._ui_ready:
            return
        total = len(self._tab_builders)
        if self._tab_build_index < total:
            position = self._tab_build_index
            builder = self._tab_builders[position]
            self.footer_status.setText(
                f"Chargement de l’interface Cartomize · {position + 1}/{total}"
            )
            try:
                builder()
            except Exception as exc:
                self.footer_status.setText(
                    f"Chargement interrompu à l’étape {position + 1}/{total} : {exc}"
                )
                QgsMessageLog.logMessage(
                    f"Construction progressive interrompue ({builder.__name__}) : {exc}",
                    "Cartomize",
                    critical_level(),
                )
                return
            self._tab_build_index += 1
            QTimer.singleShot(0, self._build_next_tab)
            return

        try:
            self._connect_signals()
            self.guided_tour = GuidedTourController(
                self,
                self._tour_parent,
                self._guided_tour_steps(),
            )
            self._ui_ready = True
            self.footer_status.setText("Interface prête")
            if self._runtime_requested:
                QTimer.singleShot(0, self._initialize_runtime_now)
        except Exception as exc:
            self.footer_status.setText(f"Initialisation interrompue : {exc}")
            QgsMessageLog.logMessage(
                f"Initialisation du panneau interrompue : {exc}",
                "Cartomize",
                critical_level(),
            )

    def initialize_runtime(self):
        """Charge le catalogue après affichage, une seule fois par panneau."""
        if self._disposed:
            return
        self._runtime_requested = True
        if not self._ui_ready:
            return
        self._initialize_runtime_now()

    def _initialize_runtime_now(self):
        if self._disposed or not self._ui_ready:
            return
        ...
        try:
            ...
            self._error("Initialisation", exc)

    def is_ready(self) -> bool:
        """Indique au chargeur que le panneau et son état QGIS sont stables."""
        return bool(self._ui_ready and self._runtime_initialized and not self._disposed)

    def schedule_refresh(self, delay_ms: int = 250):
        """Regroupe les rafales de signaux QGIS en un seul rafraîchissement."""
        if self._disposed or not self._runtime_initialized:
            return
    ...
    def dispose(self):
        ...
        task = getattr(self, "community_catalog_task", None)
        if task is not None:
            try:
                task.cancel()
            except Exception:
                logging.getLogger(__name__).debug(
                    "Unable to cancel the Cartomize community task", exc_info=True
                )
        if hasattr(self, "guided_tour"):
            self.guided_tour.dispose()
        ...
        for signal_name in ("visibilityChanged", "layerOrderChanged"):
            ...
            try:
                ...
                logging.getLogger(__name__).debug(
                    "Non-fatal Cartomize operation failed", exc_info=True
                )
    ...
    def _build_automation_tab(self):
        ...
        self.automation_visible_only = QCheckBox(
            "Utiliser les couches affichées dans QGIS"
        )
        self.automation_visible_only.setChecked(True)
        self.automation_visible_only.setToolTip(
            "Conserve l’ordre actuel des couches, y compris OpenStreetMap et "
            "les autres couches de référence."
        )
        self.automation_apply_symbology = QCheckBox(
            "Harmoniser la symbologie du projet (peut remplacer les styles actuels)"
        )
        self.automation_apply_symbology.setChecked(False)
        ...
        variant_form.setFieldGrowthPolicy(QFormLayout.FieldGrowthPolicy.AllNonFixedFieldsGrow)
        self.automation_variant_template = QComboBox()
        for spec in self.catalog.all():
            self.automation_variant_template.addItem(
                f"{spec.name} · {spec.page_format}", spec.template_id
            )
        self.automation_variant_title = QLineEdit()
        self.automation_variant_subtitle = QLineEdit()
        self.automation_variant_margin = QDoubleSpinBox()
        self.automation_variant_margin.setRange(0.0, 25.0)
        self.automation_variant_margin.setDecimals(1)
        self.automation_variant_margin.setSuffix(" %")
        self.automation_background_combo = QComboBox()
        self.automation_background_combo.setToolTip(
            "Choisissez le contexte placé derrière les données métier."
        )
        self.automation_context_opacity = QSpinBox()
        self.automation_context_opacity.setRange(0, 100)
        self.automation_context_opacity.setValue(100)
        self.automation_context_opacity.setSuffix(" %")
        self.automation_locator_combo = QComboBox()
        self._populate_locator_combo(self.automation_locator_combo)
        self.automation_variant_grid = QCheckBox("Afficher une grille de coordonnées")
        self.automation_variant_confirmed = QCheckBox("Décisions vérifiées par le cartographe")
        variant_form.addRow("Maquette", self.automation_variant_template)
        variant_form.addRow("Titre", self.automation_variant_title)
        variant_form.addRow("Sous-titre", self.automation_variant_subtitle)
        variant_form.addRow("Marge d’emprise", self.automation_variant_margin)
        variant_form.addRow(
            "Contexte cartographique", self.automation_background_combo
        )
        variant_form.addRow(
            "Opacité du contexte", self.automation_context_opacity
        )
        variant_form.addRow("Carte de situation", self.automation_locator_combo)
        variant_form.addRow("Grille", self.automation_variant_grid)
        variant_form.addRow("Validation", self.automation_variant_confirmed)
        variants_layout.addLayout(variant_form)
        ...
        self.automation_variants.currentItemChanged.connect(self._load_variant_editor)
        self.automation_visible_only.toggled.connect(
            self._refresh_layout_layer_options
        )
        self.automation_background_combo.currentIndexChanged.connect(
            lambda _index: self._context_selection_changed(
                self.automation_background_combo
            )
        )
        self.automation_context_opacity.valueChanged.connect(
            lambda value: self._context_opacity_changed(
                self.automation_background_combo,
                value,
            )
        )
    ...
    def _build_project_tab(self):
        ...
        self.import_data_button = self._style_button(
            QPushButton("Importer des données…"), "primary"
        )
        self.import_data_button.setObjectName("CartomizeImportDataButton")
        self.import_data_button.setToolTip(
            "Ouvrir le gestionnaire des sources de données de QGIS."
        )
        ...
        layer_actions.addWidget(self.import_data_button, 0, 0, 1, 2)
        layer_actions.addWidget(self.zoom_button, 1, 0)
        layer_actions.addWidget(self.properties_button, 1, 1)
        layer_layout.addLayout(layer_actions)
        layout.addWidget(layer_card)

        self.symbology_card, recommendation_layout = self._create_card(
            "Recommandation de symbologie",
            "L’analyse est lancée uniquement sur demande afin de préserver la réactivité de QGIS, y compris avec des couches volumineuses ou distantes.",
        )
        self.analyze_layer_button = self._style_button(
            QPushButton("Analyser la couche sélectionnée"), "primary"
        )
        self.analyze_layer_button.setToolTip(
            "Échantillonner la couche et préparer une recommandation modifiable."
        )
        recommendation_layout.addWidget(self.analyze_layer_button)
        ...
        self.vector_expert_editor = QFrame()
        self.vector_expert_editor.setProperty("role", "card")
        expert_form = QFormLayout(self.vector_expert_editor)
        expert_form.setFieldGrowthPolicy(QFormLayout.FieldGrowthPolicy.AllNonFixedFieldsGrow)
        self.style_mode_combo = QComboBox()
        self.style_mode_combo.addItem("Symbole unique", "Symbole unique")
        self.style_mode_combo.addItem("Catégorisé", "Catégorisé")
        self.style_mode_combo.addItem("Gradué — quantiles", "Gradué (quantiles)")
        self.style_field_combo = QComboBox()
        self.style_label_combo = QComboBox()
        self.style_class_count = QSpinBox()
        self.style_class_count.setRange(1, 100)
        self.style_palette_combo = QComboBox()
        self.style_palette_combo.addItem("Qualitative", "qualitative")
        self.style_palette_combo.addItem("Séquentielle", "séquentielle")
        self.style_palette_combo.addItem("Divergente", "divergente")
        self.style_labels_enabled = QCheckBox("Activer les étiquettes")
        self.style_label_size = QDoubleSpinBox()
        self.style_label_size.setRange(5.0, 48.0)
        self.style_label_size.setDecimals(1)
        self.style_label_size.setSuffix(" pt")
        self.style_label_placement = QComboBox()
        for label, code in (
            ("Automatique selon la géométrie", "auto"),
            ("Autour du point", "around_point"), ("Sur le point", "over_point"),
            ("Le long de la ligne", "line"), ("Courbe", "curved"),
            ("Horizontal", "horizontal"), ("Libre", "free"),
        ):
            self.style_label_placement.addItem(label, code)
        self.style_opacity = QSpinBox()
        self.style_opacity.setRange(0, 100)
        self.style_opacity.setSuffix(" %")
        self.style_expert_confirmed = QCheckBox("Confirmer les paramètres avant application")
        expert_form.addRow("Mode de rendu", self.style_mode_combo)
        expert_form.addRow("Champ thématique", self.style_field_combo)
        expert_form.addRow("Nombre maximal de classes", self.style_class_count)
        expert_form.addRow("Palette", self.style_palette_combo)
        expert_form.addRow("Champ d’étiquette", self.style_label_combo)
        expert_form.addRow("Étiquetage", self.style_labels_enabled)
        expert_form.addRow("Taille des étiquettes", self.style_label_size)
        expert_form.addRow("Placement", self.style_label_placement)
        expert_form.addRow("Opacité de la couche", self.style_opacity)
        expert_form.addRow("Validation", self.style_expert_confirmed)
        recommendation_layout.addWidget(self.vector_expert_editor)
        ...
        self.raster_intelligence_button = self._style_button(
            QPushButton("Ouvrir Raster Engine")
        )
        ...
        self.symbology_card.setObjectName("CartomizeSymbologyCard")
        layout.addWidget(self.symbology_card)
        layout.addStretch(1)

        self.tabs.addTab(scroll, "Projet")
        self.layer_combo.currentIndexChanged.connect(self._reset_recommendation)
        self.analyze_layer_button.clicked.connect(self._update_recommendation)
        self.import_data_button.clicked.connect(self.open_data_source_manager)
    ...
    def _build_layout_tab(self):
        ...
        self.template_card, template_layout = self._create_card(
            "Catalogue de maquettes",
            "Utilisez la recherche et les catégories pour parcourir les maquettes QGIS disponibles hors connexion.",
        )
        self.template_card.setObjectName("CartomizeTemplateCard")
        filters = QGridLayout()
        ...
        self.layout_content_card, settings_layout = self._create_card(
            "Contenu de la carte",
        ...
        self.context_opacity = QSpinBox()
        self.context_opacity.setRange(0, 100)
        self.context_opacity.setValue(100)
        self.context_opacity.setSuffix(" %")
        self.locator_combo = QComboBox()
        self._populate_locator_combo(self.locator_combo)
        ...
        form.addRow("Contexte cartographique", self.background_combo)
        form.addRow("Opacité du contexte", self.context_opacity)
        form.addRow("Carte de situation", self.locator_combo)
        settings_layout.addLayout(form)
        self.visible_only = QCheckBox(
            "Utiliser les couches affichées dans QGIS"
        )
        ...
        self.sync_layout_button = self._style_button(
            QPushButton("Actualiser la mise en page")
        )
        self.sync_layout_button.setToolTip(
            "Relire l’ordre et la visibilité actuels des couches QGIS, "
            "puis actualiser tous les cadres sans recréer la composition."
        )
        ...
        settings_layout.addWidget(self.sync_layout_button)
        settings_layout.addWidget(self.open_designer_button)
        self.layout_content_card.setObjectName("CartomizeLayoutContentCard")
        layout.addWidget(self.layout_content_card)

        self.export_card, output_layout = self._create_card(
            "Aperçu et export",
        ...
        self.export_card.setObjectName("CartomizeExportCard")
        ...
        self.visible_only.toggled.connect(self._refresh_layout_layer_options)
        self.background_combo.currentIndexChanged.connect(
            lambda _index: self._context_selection_changed(
                self.background_combo
            )
        )
        self.context_opacity.valueChanged.connect(
            lambda value: self._context_opacity_changed(
                self.background_combo,
                value,
            )
        ...
        self.sync_layout_button.clicked.connect(self.synchronize_selected_layout)
    ...
    def _build_quality_tab(self):
        ...
        self.quality_card, score_layout = self._create_card("Résultat du contrôle")
        self.quality_card.setObjectName("CartomizeQualityCard")
    ...
    def _build_community_tab(self):
        scroll, _tab, layout = self._create_page(
            "Communauté",
            "Accédez au portail officiel Cartomize et aux maquettes disponibles hors ligne.",
        )

        info_card, info_layout = self._create_card(
            "Portail Cartomize",
            "Consultez les maquettes publiées et les ressources de la communauté.",
        )
        community_text = QLabel(
            "Le portail s’ouvre dans votre navigateur. Aucune donnée du projet QGIS n’est transmise."
        )
        community_text.setWordWrap(True)
        info_layout.addWidget(community_text)
        layout.addWidget(info_card)

        offline_card, offline_layout = self._create_card(
            "Maquettes hors ligne",
            "24 maquettes disponibles hors ligne.",
        )
        offline_status = QLabel(
            "Elles restent accessibles lorsque le portail ou Internet est indisponible."
        )
        offline_status.setWordWrap(True)
        offline_status.setProperty("role", "muted")
        offline_layout.addWidget(offline_status)
        layout.addWidget(offline_card)

        service_card, service_layout = self._create_card(
            "Ressources en ligne",
            "Actualisez la liste des ressources publiées sur le portail officiel.",
        )
        self.community_label = QLabel()
        self.community_label.setWordWrap(True)
        service_layout.addWidget(self.community_label)
        self.community_resource_list = QListWidget()
        self.community_resource_list.setMinimumHeight(190)
        self.community_resource_list.setSelectionMode(
            QAbstractItemView.SelectionMode.SingleSelection
        )
        service_layout.addWidget(self.community_resource_list)
        self.refresh_community_button = self._style_button(
            QPushButton("Actualiser le catalogue en ligne")
        )
        self.open_community_resource_button = self._style_button(
            QPushButton("Ouvrir la ressource sélectionnée"), "primary"
        )
        self.open_community_button = self._style_button(
            QPushButton("Ouvrir le portail Cartomize")
        )
        service_layout.addWidget(self.refresh_community_button)
        service_layout.addWidget(self.open_community_resource_button)
        service_layout.addWidget(self.open_community_button)
        layout.addWidget(service_card)
        layout.addStretch(1)

        self.tabs.addTab(scroll, "Communauté")
        self.refresh_community_button.clicked.connect(self.refresh_community_catalog)
        self.open_community_resource_button.clicked.connect(self.open_community_resource)
        self.community_resource_list.itemDoubleClicked.connect(
            lambda _item: self.open_community_resource()
        )
    ...
    def _guided_tour_steps(self) -> list[GuidedTourStep]:
        """Associe chaque explication à un contrôle réellement présent."""

        return [
            GuidedTourStep(
                "Bienvenue dans Cartomize",
                "Cartomize analyse votre projet QGIS, vous aide à optimiser la représentation et produit des mises en page natives entièrement modifiables.",
                lambda: self.brand_header,
            ),
            GuidedTourStep(
                "Importer vos données",
                "Ouvrez le gestionnaire des sources QGIS pour ajouter des couches vectorielles, des rasters et les autres formats pris en charge par votre installation.",
                lambda: self.import_data_button,
                tab_index=1,
            ),
            GuidedTourStep(
                "Analyser le projet",
                "L’analyse examine les couches, leur contexte et l’objectif de la carte avant de proposer plusieurs compositions explicables.",
                lambda: self.automation_analyze_button,
                tab_index=0,
            ),
            GuidedTourStep(
                "Choisir la symbologie",
                "Le panneau expert permet de modifier le champ thématique, le mode de rendu, les classes, la palette, l’étiquetage, le placement et l’opacité avant application.",
                lambda: self.vector_expert_editor,
                tab_index=1,
            ),
            GuidedTourStep(
                "Diagnostic et recommandations",
                "Le contrôle qualité recherche les problèmes de données, de lisibilité et d’étiquetage, puis fournit des actions de correction vérifiables.",
                lambda: self.quality_card,
                tab_index=3,
            ),
            GuidedTourStep(
                "Choisir une maquette",
                "Le catalogue local contient des compositions professionnelles utilisables hors connexion. Filtrez par thème ou usage, puis choisissez la structure adaptée.",
                lambda: self.template_card,
                tab_index=2,
            ),
            GuidedTourStep(
                "Construire la mise en page",
                "Renseignez le titre, le sous-titre, les sources et la marge. La carte, la légende, l’échelle, le nord, les cadres et les textes resteront éditables dans le concepteur QGIS.",
                lambda: self.layout_content_card,
                tab_index=2,
            ),
            GuidedTourStep(
                "Exporter la carte",
                "Après vérification, exportez en PDF, SVG ou PNG, ou enregistrez la composition au format QPT pour la réutiliser dans QGIS.",
                lambda: self.export_card,
                tab_index=2,
            ),
        ]

    def maybe_start_guided_tour(self) -> bool:
        return self.guided_tour.start(force=False)

    def start_guided_tour(self) -> bool:
        return self.guided_tour.start(force=True)

    def open_data_source_manager(self):
        """Ouvre le gestionnaire QGIS, avec repli sur les actions de couche."""
        ...
        self._message(
            ...
            "Le gestionnaire des sources n’est pas disponible dans cette configuration QGIS.",
        )
    ...
    def _connect_signals(self):
        ...
        for signal_name in ("visibilityChanged", "layerOrderChanged"):
            ...
            try:
                ...
                logging.getLogger(__name__).debug(
                    "Signal d'état des couches indisponible", exc_info=True
                )

    def _project_changed(self, *_):
        # Les propositions de composition restent éditables, mais leur
        # exécution relit désormais les couches courantes. Ne pas effacer le
        # plan ici permet notamment de choisir un contexte après l'analyse.
    ...
    def refresh_all(self):
        self.settings = CartomizeSettings.load()
        configured = self.settings.community_url
        self.community_catalog = CommunityCatalogClient(
            self.settings.community_url,
            self.community_cache_path,
        )
        self._display_community_snapshot(self.community_catalog.load_cached())
        if self.community_resource_list.count() == 0:
            self.community_label.setText(
                f"Portail : {configured}\n24 maquettes disponibles hors ligne."
            )
    ...
    def generate_selected_variant(self):
        ...
        if self.current_automation_plan is None:
            return
        try:
            self._store_variant_editor()
        except Exception as exc:
            self._error("Validation de la proposition", exc)
            return
    ...
    def _execute_automation_variant(self, index: int, *, open_preview: bool):
        ...
        try:
            ...
            QApplication.processEvents()
            background_mode, background_layer_id = self._background_choice(
                self.automation_background_combo
            )
            result = self.autopilot.execute_variant(
                ...
                locator_mode=str(
                    self.automation_locator_combo.currentData() or "automatic"
                ),
            )
    ...
    def _load_variant_editor(self, current, _previous=None):
        plan = self.current_automation_plan
        if plan is None or current is None:
            return
        index = int(current.data(0, user_role()) or 0)
        if not 0 <= index < len(plan.variants):
            return
        variant = plan.variants[index]
        position = self.automation_variant_template.findData(variant.template_id)
        if position >= 0:
            self.automation_variant_template.setCurrentIndex(position)
        self.automation_variant_title.setText(variant.title)
        self.automation_variant_subtitle.setText(variant.subtitle)
        self.automation_variant_margin.setValue(float(variant.margin_percent))
        self.automation_variant_grid.setChecked(bool(variant.add_grid))
        verified = plan.confidence >= 0.70
        self.automation_variant_confirmed.setChecked(False)
        self.automation_variant_confirmed.setText(
            "J’ai vérifié les décisions proposées (facultatif à ce stade)"
            if verified else
            "Confiance limitée : je confirme ces décisions de composition"
        )

    def _store_variant_editor(self):
        plan = self.current_automation_plan
        current = self.automation_variants.currentItem()
        if plan is None or current is None:
            return
        ...
        if not 0 <= index < len(plan.variants):
            return
        if plan.confidence < 0.70 and not self.automation_variant_confirmed.isChecked():
            raise CartomizeError(
                "La confiance de la composition est limitée. Vérifiez les champs puis confirmez la décision du cartographe."
            )
        original = plan.variants[index]
        template_id = str(self.automation_variant_template.currentData() or original.template_id)
        spec = self.catalog.get(template_id)
        reasons = tuple(dict.fromkeys((*original.reasons, "Paramètres validés dans l’éditeur expert.")))
        variant = replace(
            original,
            template_id=spec.template_id,
            template_name=spec.name,
            page_format=spec.page_format,
            title=self.automation_variant_title.text().strip() or original.title,
            subtitle=self.automation_variant_subtitle.text().strip(),
            margin_percent=float(self.automation_variant_margin.value()),
            add_grid=self.automation_variant_grid.isChecked(),
            reasons=reasons,
        )
        variants = list(plan.variants)
        variants[index] = variant
        self.current_automation_plan = replace(plan, variants=tuple(variants))
        current.setText(2, variant.template_name)
        current.setText(3, variant.page_format)
    ...
    @staticmethod
    def _populate_locator_combo(combo):
        combo.clear()
        combo.addItem("Contexte adapté", "automatic")
        combo.addItem("Identique au cadre principal", "main")
        combo.addItem("Carte de référence seule", "background")
        combo.setToolTip("Définit le contenu des cadres de situation.")

    def _background_choice(self, combo):
        choice = str(combo.currentData() or "automatic")
        if choice.startswith("catalog:"):
            layer = self.project_service.activate_context_basemap(
                choice.split(":", 1)[1]
            )
            opacity_widget = self._context_opacity_widget(combo)
            if opacity_widget is not None:
                self.project_service.set_context_layer_opacity(
                    layer.id(), opacity_widget.value()
                )
            return "layer", layer.id()
        if choice.startswith("layer:"):
            return "layer", choice.split(":", 1)[1]
        return choice, ""

    def _context_opacity_widget(self, combo):
        if combo is getattr(self, "background_combo", None):
            return getattr(self, "context_opacity", None)
        if combo is getattr(self, "automation_background_combo", None):
            return getattr(self, "automation_context_opacity", None)
        return None

    def _context_peer_combo(self, combo):
        if combo is getattr(self, "background_combo", None):
            return getattr(self, "automation_background_combo", None)
        if combo is getattr(self, "automation_background_combo", None):
            return getattr(self, "background_combo", None)
        return None

    def _context_selection_changed(self, combo):
        """Matérialise immédiatement le fond choisi dans le projet QGIS."""
        ...
        try:
            ...
            if choice.startswith("catalog:"):
                ...
                layer_id = layer.id()
            elif choice.startswith("layer:"):
                layer_id = choice.split(":", 1)[1]
            elif choice == "none":
                self.project_service.clear_managed_context_basemap()

            opacity_widget = self._context_opacity_widget(combo)
            if layer_id and opacity_widget is not None:
                self.project_service.set_context_layer_opacity(
                    layer_id, opacity_widget.value()
                )

            peer = self._context_peer_combo(combo)
            if peer is not None:
                index = peer.findData(choice)
                if index >= 0:
                    peer.blockSignals(True)
                    peer.setCurrentIndex(index)
                    peer.blockSignals(False)
            QTimer.singleShot(0, self._refresh_layout_layer_options)
        except Exception as exc:
            self._error("Contexte cartographique", exc)

    def _context_opacity_changed(self, combo, value: int):
        choice = str(combo.currentData() or "automatic")
        try:
            if choice.startswith("catalog:"):
                layer = self.project_service.activate_context_basemap(
                ...
                return
            self.project_service.set_context_layer_opacity(layer_id, value)
            peer = self._context_peer_combo(combo)
            peer_opacity = self._context_opacity_widget(peer)
            if peer_opacity is not None and peer_opacity.value() != value:
                peer_opacity.blockSignals(True)
                peer_opacity.setValue(value)
                peer_opacity.blockSignals(False)
        except Exception as exc:
            self._error("Opacité du contexte", exc)

    def _refresh_background_combo(self, combo, visible_only: bool):
        previous = combo.currentData()
        if previous is None:
            stored_key = self.project_service.active_context_basemap_key()
            if stored_key:
                previous = f"catalog:{stored_key}"
            opacity_widget = self._context_opacity_widget(combo)
            if opacity_widget is not None:
                opacity_widget.blockSignals(True)
                opacity_widget.setValue(
                    self.project_service.active_context_opacity_percent()
                )
                opacity_widget.blockSignals(False)
        combo.blockSignals(True)
        combo.clear()
        combo.addItem("Selon l’affichage QGIS", "automatic")
        combo.addItem("Couches thématiques uniquement", "none")
        for definition in self.project_service.context_basemap_definitions():
            combo.addItem(definition.label, f"catalog:{definition.key}")
        visible_ids = {
            layer.id() for layer in self.project_service.visible_layers()
        }
        for layer in self.project_service.background_candidates():
            if visible_only and layer.id() not in visible_ids:
                continue
            combo.addItem(layer.name(), f"layer:{layer.id()}")
        index = combo.findData(previous)
        combo.setCurrentIndex(index if index >= 0 else 0)
        combo.blockSignals(False)

    def _refresh_layout_layer_options(self):
        """Actualise les couches de référence sans modifier le choix courant."""

        if hasattr(self, "background_combo"):
            self._refresh_background_combo(
                self.background_combo, self.visible_only.isChecked()
            )
        if hasattr(self, "automation_background_combo"):
            self._refresh_background_combo(
                self.automation_background_combo,
                self.automation_visible_only.isChecked(),
            )
    ...
    def _update_recommendation(self):
        ...
        if (
            not isinstance(layer, (QgsVectorLayer, QgsRasterLayer))
        ...
        if self.project_service.is_basemap_layer(layer):
            self.current_vector_profile = None
            self.current_style_recommendation = None
            self.vector_expert_editor.setVisible(False)
            self.recommendation.setPlainText(
                "Cette couche est un fond cartographique distant. Son rendu "
                "d'origine est protégé. Utilisez-la dans la rubrique "
                "Arrière-plan de la mise en page, sans lui appliquer de "
                "classification."
            )
            self.apply_style_button.setEnabled(False)
            self.undo_style_button.setEnabled(False)
        ...
        try:
            if isinstance(layer, QgsRasterLayer):
                ...
                self.vector_expert_editor.setVisible(True)
                ...
                self.current_style_recommendation = recommendation
                self._populate_vector_expert_editor(layer, recommendation)
                rationale = "".join(
                    f"<p style='margin:4px 0'>{escape(line)}</p>"
                    for line in recommendation.rationale
                )
    ...
    def _reset_recommendation(self, *_):
        """Réinitialise l'aperçu sans parcourir les entités de la couche."""
        self.current_vector_profile = None
        self.current_style_recommendation = None
        layer = self.selected_layer()
        valid = isinstance(layer, (QgsVectorLayer, QgsRasterLayer)) and layer.isValid()
        self.analyze_layer_button.setEnabled(valid)
        self.apply_style_button.setEnabled(False)
        self.undo_style_button.setEnabled(False)
        self.raster_intelligence_button.setEnabled(
        ...
        if valid:
            self.recommendation.setPlainText(
                ...
                "Aucune entité n’est parcourue automatiquement."
            )
        else:
            self.recommendation.setPlainText(
    ...
    def _populate_vector_expert_editor(self, layer: QgsVectorLayer, recommendation) -> None:
        """Expose chaque hypothèse automatique avant l'application du style."""
        fields = list(layer.fields())
        self.style_field_combo.blockSignals(True)
        self.style_label_combo.blockSignals(True)
        self.style_field_combo.clear()
        self.style_label_combo.clear()
        self.style_label_combo.addItem("Aucune étiquette", "")
        for field in fields:
            self.style_field_combo.addItem(field.name(), field.name())
            self.style_label_combo.addItem(field.name(), field.name())
        for combo, value in (
            (self.style_mode_combo, recommendation.mode),
            (self.style_field_combo, recommendation.field_name),
            (self.style_label_combo, recommendation.label_field),
            (self.style_palette_combo, recommendation.palette),
            (self.style_label_placement, recommendation.label_placement),
        ):
            index = combo.findData(value)
            if index >= 0:
                combo.setCurrentIndex(index)
        self.style_class_count.setValue(max(1, int(recommendation.class_count)))
        self.style_labels_enabled.setChecked(
            bool(recommendation.labels_enabled and recommendation.label_field)
        )
        self.style_label_size.setValue(float(recommendation.label_font_size_pt))
        self.style_opacity.setValue(int(recommendation.opacity_percent))
        verified = recommendation.confidence >= 0.65
        self.style_expert_confirmed.setChecked(False)
        self.style_expert_confirmed.setText(
            "J’ai vérifié cette proposition (facultatif lorsque la confiance est suffisante)"
            if verified else "Confiance faible : je confirme explicitement ces choix"
        )
        self.style_field_combo.blockSignals(False)
        self.style_label_combo.blockSignals(False)
    ...
    def _apply_recommendation(self):
        ...
        try:
            if self.project_service.is_basemap_layer(layer):
                raise CartomizeError(
                    "Le rendu du fond cartographique est protégé. "
                    "Sélectionnez une couche thématique pour la symbologie."
                )
            if isinstance(layer, QgsRasterLayer):
                ...
                if profile is None and isinstance(layer, QgsVectorLayer):
                    profile = self.autopilot.geo.vector.analyze(layer)
                recommendation = self.current_style_recommendation or self.symbology.recommend_from_profile(layer, profile)
                mode = str(self.style_mode_combo.currentData() or "Symbole unique")
                recommendation = replace(
                    recommendation,
                    mode=mode,
                    field_name="" if mode == "Symbole unique" else str(self.style_field_combo.currentData() or ""),
                    label_field=str(self.style_label_combo.currentData() or ""),
                    class_count=int(self.style_class_count.value()),
                    palette=str(self.style_palette_combo.currentData() or "qualitative"),
                    labels_enabled=self.style_labels_enabled.isChecked(),
                    label_font_size_pt=float(self.style_label_size.value()),
                    label_placement=str(self.style_label_placement.currentData() or "auto"),
                    opacity_percent=int(self.style_opacity.value()),
                    expert_confirmed=self.style_expert_confirmed.isChecked(),
                )
    ...
    def _open_raster_intelligence(self):
        ...
        if not isinstance(layer, QgsRasterLayer) or not layer.isValid():
            self._message("Raster Engine", "Sélectionnez une couche raster valide.")
            return
        dialog = RasterIntelligenceDialog(
            layer, self.raster_symbology.intelligence, self,
            symbology_service=self.raster_symbology,
        )
        dialog_exec(dialog)
        self._update_recommendation()
        self.refresh_layouts()
    ...
    def _show_template_details(self, current, _previous=None):
        ...
        self.template_details.setHtml(
            ...
            "<tr><td><b>Disponibilité</b></td><td>Hors connexion</td></tr>"
            "</table>"
            f"<ul>{notes}</ul>"
        )
    ...
    def _current_layout_options(self) -> LayoutBuildOptions:
        """Capture les réglages et les couches au moment de l'action."""

        self._refresh_layout_layer_options()
        main_layer = self.selected_layer() or self.iface.activeLayer()
        background_mode, background_layer_id = self._background_choice(
            self.background_combo
        )
        return LayoutBuildOptions(
            ...
            include_visible_context=self.visible_only.isChecked(),
            background_mode=background_mode,
            background_layer_id=background_layer_id,
            locator_mode=str(
                self.locator_combo.currentData() or "automatic"
            ),
        )

    def synchronize_selected_layout(self):
        """Reconstruit les piles des cadres depuis l'état actuel de QGIS."""

        layout = self.selected_layout()
        if layout is None:
            self._message(
                "Actualiser la mise en page",
                "Sélectionnez d’abord une mise en page Cartomize.",
            )
            return
        try:
            result = self.builder.synchronize_with_project(
                layout,
                self._current_layout_options(),
            )
            detail = (
                f"{result.map_count} cadre(s) actualisé(s) avec "
                f"{result.main_layer_count} couche(s) dans le cadre principal."
            )
            if result.warnings:
                detail += " " + " ".join(result.warnings)
            self._message("Mise en page actualisée", detail, True)
            self.preview.open(
                layout,
                self.settings.preview_width_px,
                zoom_mode="width",
            ...
            self._error("Actualiser la mise en page", exc)
    ...
    def refresh_community_catalog(self):
        """Actualise les seules métadonnées publiques dans une tâche QGIS."""

        current = getattr(self, "community_catalog_task", None)
        if current is not None and current.isActive():
            return
        self.refresh_community_button.setEnabled(False)
        self.community_label.setText(
            "Actualisation du portail… 24 maquettes disponibles hors ligne."
        )
        base_url = self.settings.community_url
        cache_path = self.community_cache_path

        def fetch(_task):
            return CommunityCatalogClient(base_url, cache_path).refresh()

        def finished(exception, result=None):
            self.refresh_community_button.setEnabled(True)
            self.community_catalog_task = None
            if exception is not None or not isinstance(result, CommunityCatalogSnapshot):
                cached = self.community_catalog.load_cached()
                warning = "Le portail est indisponible. Le fonctionnement hors connexion est maintenu."
                self._display_community_snapshot(
                    CommunityCatalogSnapshot(
                        cached.resources, cached.fetched_at, cached.source, warning
                    )
                )
                return
            self._display_community_snapshot(result)

        self.community_catalog_task = QgsTask.fromFunction(
            "Actualiser le catalogue Cartomize",
            fetch,
            on_finished=finished,
        )
        QgsApplication.taskManager().addTask(self.community_catalog_task)

    def _display_community_snapshot(self, snapshot: CommunityCatalogSnapshot):
        if not hasattr(self, "community_resource_list"):
            return
        selected_item = self.community_resource_list.currentItem()
        selected_url = selected_item.data(user_role()) if selected_item else ""
        self.community_resource_list.clear()
        for resource in snapshot.resources:
            detail = " · ".join(
                part
                for part in (
                    resource.page_format,
                    resource.resource_format.upper(),
                    resource.category.replace("_", " ").capitalize(),
                )
                if part
            )
            item = QListWidgetItem(
                f"{resource.title}" + (f" — {detail}" if detail else "")
            )
            item.setData(user_role(), resource.detail_url)
            item.setToolTip(resource.description)
            self.community_resource_list.addItem(item)
            if resource.detail_url == selected_url:
                self.community_resource_list.setCurrentItem(item)
        if self.community_resource_list.currentRow() < 0 and self.community_resource_list.count():
            self.community_resource_list.setCurrentRow(0)
        configured = self.settings.community_url
        if snapshot.source == "network":
            state = f"{len(snapshot.resources)} ressources en ligne actualisées."
        elif snapshot.source == "cache":
            state = f"{len(snapshot.resources)} ressources affichées depuis le dernier cache."
        else:
            state = "Aucune ressource en ligne mémorisée."
        if snapshot.warning:
            state += f" {snapshot.warning}"
        self.community_label.setText(
            f"Portail : {configured}\n{state}\n24 maquettes disponibles hors ligne."
        )

    def open_community_resource(self):
        item = self.community_resource_list.currentItem()
        if item is None:
            self._message("Communauté", "Sélectionnez une ressource du portail.")
            return
        try:
            self.community.open_in_browser(str(item.data(user_role()) or ""))
        except Exception as exc:
            self._error("Communauté", exc)
    ...
    def open_settings(self):
        ...
        if dialog_exec(dialog):
            ...
            if dialog.wants_guided_tour():
                QTimer.singleShot(0, self.start_guided_tour)
