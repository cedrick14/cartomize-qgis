...
class AutomationRecipe:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "AutomationRecipe":
        ...
        return cls(
            ...
            background_mode=str(payload.get("background_mode") or "automatic"),
            background_layer_id=str(payload.get("background_layer_id") or ""),
            locator_mode=str(payload.get("locator_mode") or "automatic"),
        )
...
class CartomizeAutopilot:
    ...
    def analyze(
        ...
        thematic_layers = [
            layer
            for layer in layers
            if not self.project_service.is_basemap_layer(layer)
        ]
        if not thematic_layers:
            raise CartomizeError(
                "Chargez au moins une couche thématique en plus du fond de carte."
            )

        visible_ids = {layer.id() for layer in self.project_service.visible_layers()}
        profiles = tuple(
            self._profile_layer(layer, layer.id() in visible_ids)
            for layer in thematic_layers
        )
        main = self._select_main_layer(thematic_layers, main_layer_id)
        detected, confidence, reason = self._detect_objective(objective, profiles, main)
        geo_report = self.geo.analyze(
            thematic_layers,
            main_layer_id=main.id(),
            objective=detected,
        )
        warnings = tuple(dict.fromkeys(
            (*self._project_warnings(thematic_layers), *geo_report.warnings)
        ))
    ...
    def execute_variant(
        ...
        *,
        ...
        # Le plan explique une proposition, mais ne doit jamais figer l'état
        # des couches. Au moment exact de l'exécution, relire le canevas et
        # l'arbre QGIS évite de restaurer une couche masquée, supprimée ou un
        # ancien fond cartographique.
        if visible_only:
            project_layers = [
                layer for layer in self.project_service.visible_layers()
                if layer is not None and layer.isValid()
            ]
        else:
            project_layers = [
                layer for layer in self.project_service.ordered_layers()
                if layer is not None and layer.isValid()
            ]
        execution_layer_ids = tuple(layer.id() for layer in project_layers)
        thematic_layers = [
            layer
        ...
        if not thematic_layers:
            raise CartomizeError(
                "La recette ne contient plus de couche thématique valide."
            )
        execution_main = self.project_service.focus_layer(
            thematic_layers,
            plan.main_layer_id,
        )
        execution_main_id = (
            execution_main.id() if execution_main is not None else ""
        )
        execution_main_name = (
            execution_main.name() if execution_main is not None else ""
        )
        geo_report = self.geo.analyze(
            thematic_layers,
            main_layer_id=execution_main_id,
        ...
        options = LayoutBuildOptions(
            ...
            open_designer=False,
            layer_ids=execution_layer_ids,
            main_layer_id=execution_main_id,
            include_visible_context=visible_only,
    ...
    def replay_recipe(self, recipe: AutomationRecipe) -> AutomationExecutionResult:
        ...
        return self.execute_variant(
            ...
            background_layer_id=recipe.background_layer_id,
            locator_mode=recipe.locator_mode,
        )
    ...
    def _profile_layer(self, layer, visible: bool) -> LayerProfile:
        if isinstance(layer, QgsVectorLayer):
            ...
            numeric: list[str] = []
            for layer_field in list(layer.fields())[:250]:
                if layer_field.isNumeric():
                    numeric.append(layer_field.name())
                elif not layer_field.isDateOrTime():
                    categorical.append(layer_field.name())
            try:
            ...
            if str(layer.customProperty(
                "cartomize/expert_labels_configured", False
            )).casefold() in {"1", "true", "yes"}:
                expert_field = str(layer.customProperty(
                    "cartomize/expert_label_field", ""
                ) or "")
                if not expert_field or layer.fields().indexOf(expert_field) >= 0:
                    label_field = expert_field
            try:
    ...
    def _select_main_layer(self, layers: list, requested_id: str):
        ...
        candidates = thematic_layers or list(layers)
        if requested_id:
            requested = next(
                (layer for layer in candidates if layer.id() == requested_id),
                None,
            )
        ...
        if active is not None and active in candidates:
            return active
        scored = sorted(candidates, key=self._main_layer_score, reverse=True)
        return scored[0]
    ...
    def _main_layer_score(layer) -> float:
        score = 0.0
        try:
            if is_remote_basemap(layer.providerType(), layer.source(), layer.name()):
                return -100.0
        except Exception:
            logging.getLogger(__name__).debug(
                "Identification du fond de carte impossible", exc_info=True
            )
        if isinstance(layer, QgsRasterLayer):
            score += 8.0
        if isinstance(layer, QgsVectorLayer):
    ...
    def _detect_objective(
        ...
        project_corpus = " ".join(
            ...
            + [field_name for profile in profiles for field_name in (*profile.categorical_fields, *profile.numeric_fields)]
        ).casefold()
    ...
    def _legend_complexity(geo_report) -> int:
        ...
        try:
            for profile in geo_report.vector_profiles:
                thematic_field = next(
                    (item for item in profile.fields if item.name == profile.thematic_field),
                    None,
                )
                if thematic_field is not None and thematic_field.semantic_role in {"category", "coded_category", "ordinal"}:
                    complexity = max(complexity, min(40, int(thematic_field.unique_count)))
        except Exception:
            logging.getLogger(__name__).debug("Non-fatal Cartomize operation failed", exc_info=True)
        return complexity
