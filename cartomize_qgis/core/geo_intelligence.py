...
class GeoIntelligenceEngine:
    ...
    def analyze(self, layers: Iterable, *, main_layer_id: str, objective: str) -> GeoIntelligenceReport:
        ...
        for profile in vector_profiles:
            ...
            label_recommendations.append(_with_expert_label_choice(layer, recommendation))
    ...
    def apply_layout_scale(self, layout, report: GeoIntelligenceReport) -> tuple[str, ...]:
        ...
        for layer_id, profile in profiles.items():
            ...
            recommendation = _with_expert_label_choice(layer, recommendation)
            try:
                if scale_rec.visible and self.labels.apply(layer, recommendation):
                    action = "étiquetage" if recommendation.enabled else "étiquetage désactivé"
                    changes.append(f"{action} {layer.name()} à 1:{scale_value:,.0f}")
...
def _with_expert_label_choice(
    layer: QgsVectorLayer,
    recommendation: LabelRecommendation,
) -> LabelRecommendation:
    """Réinjecte les paramètres explicitement validés dans les analyses futures."""
    configured = layer.customProperty("cartomize/expert_labels_configured", False)
    if str(configured).casefold() not in {"1", "true", "yes"} and configured is not True:
        return recommendation
    enabled_raw = layer.customProperty("cartomize/expert_labels_enabled", True)
    enabled = enabled_raw is True or str(enabled_raw).casefold() in {"1", "true", "yes"}
    field_name = str(layer.customProperty("cartomize/expert_label_field", "") or "")
    if field_name and layer.fields().indexOf(field_name) < 0:
        return recommendation
    try:
        size = float(layer.customProperty(
            "cartomize/expert_label_size_pt", recommendation.font_size_pt
        ))
    except (TypeError, ValueError):
        size = recommendation.font_size_pt
    placement = str(layer.customProperty(
        "cartomize/expert_label_placement", recommendation.placement
    ) or recommendation.placement)
    return replace(
        recommendation,
        field_name=field_name,
        enabled=bool(enabled and field_name),
        font_size_pt=max(5.0, min(48.0, size)),
        placement=placement,
        confidence=1.0,
        reason="Choix d'étiquetage validé par l'expert et conservé par Cartomize.",
    )
