...
class ProjectStylingOrchestrator:
        ...
        *,
        ...
        for layer in layers:
            if not layer or not layer.isValid():
                continue
            if self._is_protected_basemap(layer):
                decisions.append(
                    StylingDecision(
                        layer.id(),
                        layer.name(),
                        "fond",
                        "Style du fournisseur conservé",
                        1.0,
                        (
                            "Les fonds XYZ, WMTS, OpenStreetMap et QuickMapServices "
                            "restent dans la pile cartographique sans être reclassés."
                        ),
                        False,
                    )
                )
    ...
    @staticmethod
    def _is_protected_basemap(layer) -> bool:
        """Empêche toute mutation du renderer des fonds fournis par un service web."""

        try:
            return is_remote_basemap(
                layer.providerType(),
                layer.source(),
                layer.name(),
            )
    ...
    def _apply_context_style(layer: QgsVectorLayer, role: str) -> bool:
        ...
        if geometry == QgsWkbTypes.GeometryType.LineGeometry:
            ...
            geometry == QgsWkbTypes.GeometryType.PolygonGeometry
            and role == "limites"
        ):
            ...
            geometry == QgsWkbTypes.GeometryType.PointGeometry
            and role == "localités"
        ):
