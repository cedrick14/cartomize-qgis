"""Graphe explicable des relations spatiales et cartographiques entre couches."""
...
class ProjectRelationshipEngine:
    """Construit un graphe léger à partir des emprises, rôles et types de couches."""
        ...
        *,
        ...
        for layer in layers:
            if layer is None or not layer.isValid():
                continue
            role = "principal" if layer.id() == main_layer_id else roles.get(layer.id(), "contexte")
            extent = self._extent_in_project_crs(layer, project_crs)
            layer_type = "vector" if isinstance(layer, QgsVectorLayer) else "raster" if isinstance(layer, QgsRasterLayer) else "other"
            priority = _ROLE_PRIORITY.get(role, 40)
            nodes.append(
                LayerNode(
                    str(layer.id()), str(layer.name()), layer_type, role,
                    layer.crs().authid() or layer.crs().description() or "",
                    extent, priority,
                )
            )
        ...
        context = tuple(
            ...
            if node.layer_id != main_layer_id and node.role in {"limites", "transport", "hydrographie", "localités", "contexte"}
        )
        return ProjectRelationshipGraph(tuple(nodes), tuple(relations), ordered, context)

    def _extent_in_project_crs(self, layer, project_crs) -> tuple[float, float, float, float]:
        try:
            extent = layer.extent()
            if layer.crs().isValid() and project_crs.isValid() and layer.crs() != project_crs:
                transform = QgsCoordinateTransform(layer.crs(), project_crs, self.project.transformContext())
                extent = transform.transformBoundingBox(extent)
            return (
                float(extent.xMinimum()), float(extent.yMinimum()),
...
def _classify_extent_relation(source: LayerNode, target: LayerNode) -> LayerRelation | None:
    ...
    return LayerRelation(source.layer_id, target.layer_id, relation, round(min(ratio_a, ratio_b), 4), round(confidence, 3), explanation)
