"""Catalogue sécurisé des maquettes Cartomize disponibles hors ligne.
...
from .constants import (
    ...
    OFFLINE_TEMPLATE_COUNT,
    OFFLINE_TEMPLATE_MANIFEST,
...
class TemplateCatalog:
    ...
    def reload(self) -> None:
        ...
        manifest = self.root / OFFLINE_TEMPLATE_MANIFEST
        entries = self._manifest_entries(manifest) if manifest.is_file() else None
        paths = (
            [(self.root / relative).resolve() for relative in entries]
            if entries is not None
            else [path.resolve() for path in sorted(self.root.rglob("*.json")) if path.name != OFFLINE_TEMPLATE_MANIFEST]
        )
        for resolved in paths:
            ...
            if self.root not in resolved.parents:
                raise TemplateError("Chemin de maquette hors du catalogue local.")
            if not resolved.is_file():
                raise TemplateError(f"Maquette hors ligne introuvable : {resolved.name}")
    ...
    def _manifest_entries(self, path: Path) -> list[str]:
        """Valide la sélection embarquée sans accepter de chemin arbitraire."""

        if path.stat().st_size > MAX_TEMPLATE_BYTES:
            raise TemplateError("Le manifeste des maquettes hors ligne est trop volumineux.")
        try:
            raw = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError) as exc:
            raise TemplateError("Le manifeste des maquettes hors ligne est invalide.") from exc
        items = raw.get("templates") if isinstance(raw, dict) else None
        if not isinstance(items, list) or len(items) != OFFLINE_TEMPLATE_COUNT:
            raise TemplateError(
                f"Le catalogue hors ligne doit contenir exactement {OFFLINE_TEMPLATE_COUNT} maquettes."
            )
        result: list[str] = []
        seen: set[str] = set()
        for item in items:
            if not isinstance(item, dict):
                raise TemplateError("Entrée de manifeste hors ligne invalide.")
            ...
            if (
                ...
                raise TemplateError(f"Entrée de manifeste interdite : {relative or '<vide>'}")
            seen.add(relative)
