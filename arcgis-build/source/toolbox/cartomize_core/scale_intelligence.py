...
class ScaleIntelligenceEngine:
    ...
    def analyze_layer(self, layer, role: str, scale: float) -> ScaleRecommendation:
        ...
        return ScaleRecommendation(
            layer_id=_layer_id(layer), role=role, scale=scale, visible=visible,
    ...
    def analyze_project(self, layers, roles: dict[str, str], scale: float) -> tuple[ScaleRecommendation, ...]:
        return tuple(
            self.analyze_layer(layer, roles.get(_layer_id(layer), "contexte"), scale)
            for layer in layers if layer is not None and _layer_is_valid(layer)
        )
...
def _layer_id(layer) -> str:
    """Identifiant stable compatible avec QgsMapLayer et arcpy.mp.Layer."""

    value = getattr(layer, "URI", None)
    if value:
        return str(value)
    candidate = getattr(layer, "id", None)
    if callable(candidate):
        try:
            return str(candidate())
        except Exception:
            pass
    return str(candidate or getattr(layer, "name", ""))


def _layer_is_valid(layer) -> bool:
    valid = getattr(layer, "isValid", None)
    if callable(valid):
        try:
            return bool(valid())
        except Exception:
            return False
    return not bool(getattr(layer, "isBroken", False))
