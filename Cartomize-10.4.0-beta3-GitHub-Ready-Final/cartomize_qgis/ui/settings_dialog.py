...
from ..core.settings import CartomizeSettings, validate_community_url
...
class SettingsDialog(QDialog):
    def __init__(self, settings: CartomizeSettings, parent=None):
        ...
        self.community_url = QLineEdit(settings.community_url)
        self.community_url.setPlaceholderText("https://community.cartomize.org")
    ...
    def _general_tab(self):
        ...
        community, community_layout = self._card(
            "Communauté",
            "L’adresse doit utiliser HTTPS. Aucune donnée du projet n’est envoyée automatiquement.",
        )
        community_form = QFormLayout()
        community_form.setFieldGrowthPolicy(QFormLayout.AllNonFixedFieldsGrow)
        community_form.addRow("Adresse du service", self.community_url)
    ...
    def _accept_validated(self):
        try:
            validate_community_url(self.community_url.text())
        except ValueError:
            self.community_url.setFocus()
            self.community_url.setStyleSheet("border: 1px solid #dc2626;")
            return
        self.community_url.setStyleSheet("")
        self.accept()
    ...
    def result_settings(self) -> CartomizeSettings:
        return CartomizeSettings(
            ...
            community_url=validate_community_url(self.community_url.text()),
