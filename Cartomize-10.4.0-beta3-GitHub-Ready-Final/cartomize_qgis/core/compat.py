...
def preview_dpi_for_width(layout, target_width_px: int) -> int:
    """Calcule le DPI nécessaire pour rendre la première page à la largeur demandée."""
...
def configure_layout_rendering(layout, dpi: int) -> None:
    """Configure un aperçu net et des textes vectoriels, avec compatibilité QGIS 3.40+."""
    ...
    modern_flags = getattr(Qgis, "LayoutRenderFlag", None)
    for name in (
        "Antialiasing",
        "UseAdvancedEffects",
        ...
        # Active explicitement les drapeaux de haute qualité quand QGIS les expose.
        flag = getattr(modern_flags, name, None) if modern_flags is not None else None
        if flag is None:
...
def right_dock_area():
    dock = getattr(Qt, "DockWidgetArea", None)
    if dock is not None:
        return dock.RightDockWidgetArea
    return Qt.RightDockWidgetArea


def user_role():
    roles = getattr(Qt, "ItemDataRole", None)
...
def checked_state():
    states = getattr(Qt, "CheckState", None)
        ...
        for project_key in (key, f"/{key}"):
            try:
                ...
                if isinstance(result, tuple):
                    value, ok = result[0], bool(result[1]) if len(result) > 1 else True
                    if ok:
                        return value
                elif result != default:
                    return result
