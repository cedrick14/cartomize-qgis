...
def detect_nodata_candidates(evidence: RasterEvidence) -> tuple[RasterCandidate, ...]:
    ...
    for profile in evidence.values:
        ...
        if profile.border_percentage >= 0.65 and profile.center_percentage <= 0.25 and border_signal >= 0.45:
            confidence = min(0.96, 0.62 + border_signal * 0.38)
            reason = "Valeur concentrée sur la bordure et rare au centre du raster."
